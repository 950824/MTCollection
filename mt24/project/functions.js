var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = 
{
	...
	if (core.status.played){
		console.log(core.status.played)
		window.mota_Android.ShowFloat("1")
	}
	// 初始化人物，图标，统计信息
	...
	if (x != null && y != null) {
		...
		if (guards.length > 0) {
			...
			core.push(actions, [
				...
				{ "type": "setBlock", "number": enemyId, "loc": [
						[x, y]
					] }, // 重新设置怪物自身
	...
	if (equipId && (core.material.items[equipId].equip || {}).animate)
		var animate = core.material.items[equipId].equip.animate;
	...
	*/
	...
	if (!core.domStyle.isVertical) {
		...
		// 绘制图标
		core.drawImage(ctx, core.statusBar.icons.hpmax, 6, 9, 25, 25);
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus("lv")), 52, 30);
		_fillBoldTextWithFontCheck('级', 100, 30);

		// 绘制生命
		_fillBoldTextWithFontCheck('生命', 6, 80);
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('hp')), 50, 80);

		// 绘制攻击
		_fillBoldTextWithFontCheck('攻击', 6, 110);
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('atk')), 60, 110);

		// 绘制防御
		_fillBoldTextWithFontCheck('防御', 6, 140);
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('def')), 60, 140);



		// 绘制金币
		_fillBoldTextWithFontCheck('金币', 6, 170);
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('money')), 60, 170);

		// 绘制经验
		_fillBoldTextWithFontCheck('经验', 6, 200);
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('exp')), 60, 200);

		// 绘制三色钥匙
		core.drawImage(ctx, 'yellowKey.png', 10, 220, 32, 32)
		_fillBoldTextWithFontCheck(core.setTwoDigits(core.itemCount('yellowKey')), 60, 240, '#FFCCAA');
		_fillBoldTextWithFontCheck('个', 100, 240, 'white');
		core.drawImage(ctx, 'blueKey.png', 10, 270, 32, 32)
		_fillBoldTextWithFontCheck(core.setTwoDigits(core.itemCount('blueKey')), 60, 290, '#AAAADD');
		_fillBoldTextWithFontCheck('个', 100, 290, 'white');
		core.drawImage(ctx, 'redKey.png', 10, 320, 32, 32)
		_fillBoldTextWithFontCheck(core.setTwoDigits(core.itemCount('redKey')), 60, 340, '#FF8888');
		_fillBoldTextWithFontCheck('个', 100, 340, 'white');
		//绘制楼层
		_fillBoldTextWithFontCheck((core.status.thisMap || {}).name || "", 30, 390);
	} else {
		// 竖屏模式

		// 绘制图标
		core.drawImage(ctx, core.statusBar.icons.hpmax, 6, 9, 25, 25);
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus("lv")), 50, 30);
		_fillBoldTextWithFontCheck('级', 90, 30)
		// 绘制生命
		_fillBoldTextWithFontCheck('生命', 150, 30);
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('hp')), 200, 30);

		// 绘制攻击
		_fillBoldTextWithFontCheck('攻击', 90, 65);
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('atk')), 150, 65);

		// 绘制防御
		_fillBoldTextWithFontCheck('防御', 90, 95);
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('def')), 150, 95);



		// 绘制金币
		_fillBoldTextWithFontCheck('金币', 215, 65);
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('money')), 265, 65);

		// 绘制经验
		_fillBoldTextWithFontCheck('经验', 215, 95);
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('exp')), 265, 95);
		// 绘制三色钥匙
		core.drawImage(ctx, 'yellowKey.png', 330, 10, 32, 32)
		_fillBoldTextWithFontCheck(core.setTwoDigits(core.itemCount('yellowKey')), 365, 30, '#FFCCAA');
		_fillBoldTextWithFontCheck('个', 395, 30, 'white');
		core.drawImage(ctx, 'blueKey.png', 330, 40, 32, 32)
		_fillBoldTextWithFontCheck(core.setTwoDigits(core.itemCount('blueKey')), 365, 60, '#AAAADD');
		_fillBoldTextWithFontCheck('个', 395, 60, 'white');
		core.drawImage(ctx, 'redKey.png', 330, 70, 32, 32)
		_fillBoldTextWithFontCheck(core.setTwoDigits(core.itemCount('redKey')), 365, 90, '#FF8888');
		_fillBoldTextWithFontCheck('个', 395, 90, 'white');
		//绘制楼层
		_fillBoldTextWithFontCheck((core.status.thisMap || {}).name || "", 10, 75);
